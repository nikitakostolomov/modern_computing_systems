import random
from tkinter import (RIGHT, VERTICAL, Button, Canvas, Entry, Frame, Label,
                     OptionMenu, Scrollbar, StringVar, W, Y)
...
class ConsistentFrame(Frame):
    ...
    def method_dropdown(self) -> None:
        self.__dropdown(
            self.tools_frame, "Method", self.__state["method"], list(features.keys())
        )
    ...
    def __dropdown(
        self, frame: Frame, label: str, var: StringVar, options: list
    ) -> None:
        Label(frame, text=label).pack()
        OptionMenu(frame, var, *options).pack()

    def __entry(
        self, frame: Frame, label: str, var: StringVar, default: str = ""
    ) -> None:
        Label(frame, text=label).pack()
